...
###############################################################################
# Fixed Optimized Implementation for Training
###############################################################################
class OptimizedPartialConv2dFixed(nn.Conv2d):
    ...
    def forward(
        ...
        if self.bias is not None:
            # FIXED: Dynamic bias view for proper gradient flow
...
class TrainingBenchmarkResult:
    """Results from training benchmark"""
...
def benchmark_training_step(
    ...
    """Benchmark a single training step including forward and backward passes"""
...
def main():
    """Run training benchmark with a single test case"""
    ...
    nvidia_layer = NvidiaPartialConv2d(
    ...
    optimized_layer = OptimizedPartialConv2dFixed(
        in_channels=in_channels,
        out_channels=out_channels,
        kernel_size=kernel_size,
        padding=padding,
    ...
    mask_configs = [
        (
            ...
            (torch.rand(batch_size, 1, height, width, device=device) > 0.2).float(),
        ),
        (
            "Random mask (50% valid)",
            (torch.rand(batch_size, 1, height, width, device=device) > 0.5).float(),
        ),
        ("Full mask", torch.ones(batch_size, 1, height, width, device=device)),
    ]

    # Target for loss calculation (same size as output)
    target = torch.randn(batch_size, out_channels, height, width, device=device)

    # Loss function
    loss_fn = nn.MSELoss()

    # Run benchmark for each mask type
    for mask_name, mask in mask_configs:
        print(f"\n{'='*80}")
        print(f"Testing with: {mask_name}")
        print(f"{'='*80}")
        ...
        # Print summary for this mask type
        print("\n" + "=" * 80)
        print(f"TRAINING BENCHMARK SUMMARY - {mask_name}")
        print("=" * 80)
        ...
        # Only run one configuration if you want a quick test
        break  # Remove this line to test all mask types
...
###############################################################################
# NVIDIA Original Implementation (for comparison)
###############################################################################
